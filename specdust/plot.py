...
d=[-218.4458, 0.4004898, 0.1778779, 9.7997800E-02, 3.9183609E-02, 3.8629450E-02, 3.1717252E-02, 4.2692993E-02, 4.3908957E-02, 5.9714161E-02, 9.9677220E-02, 0.1017583, 9.8673873E-02, 0.1004481, 0.1267319, 0.1271955, 0.1258029, 0.1274267, 0.1272058, 0.1270577, 0.1268518, 0.1289727, 0.2887138]
f=[0.408, 22.8, 28.4, 33., 40.6, 40.6, 44.1, 60.8, 60.8, 70.3, 93.5, 93.5, 93.5, 93.5, 100., 100., 100., 100., 100., 100., 100., 100., 143.]
bp = [0.000, 0.206400, 0.030700,-0.777600,-1.267200,-1.004800,-1.170700,0.000000,0.000000,0.984700,-2.380800,-1.910400,-2.035200,-2.284800,0.022500,-0.540900,-2.209200,-1.22090,-1.731400,-0.694000,-2.200100,-1.419700,0.403600]
f_corr=[x + y for x, y in zip(f,bp)]
...
meanf=[0.408, 22.8, 28.4, 33., 40.6,44.1, 60.8, 70.3, 93.5, 100., 143.]
meand=[107.3353, 0.4481514, 0.1992134, 0.1069137, np.mean([4.3700784E-02, 4.3227028E-02]), 3.5943400E-02, np.mean([4.4008739E-02, 4.5086958E-02]), 6.2040720E-02, np.mean([0.1021026, 0.1042721, 0.1023647, 0.1032797]), np.mean([0.1290995, 0.1294701, 0.1288557, 0.1302578, 0.1299172, 0.1297056, 0.1292863, 0.1312539]), 0.2923834]
mediand=[107.3353, 0.4481514, 0.1992134, 0.1069137, np.median([4.3700784E-02, 4.3227028E-02]), 3.5943400E-02, np.median([4.4008739E-02, 4.5086958E-02]), 6.2040720E-02, np.median([0.1021026, 0.1042721, 0.1023647, 0.1032797]), np.median([0.1290995, 0.1294701, 0.1288557, 0.1302578, 0.1299172, 0.1297056, 0.1292863, 0.1312539]), 0.2923834]


dir = "../masterplots/figs/spec_param/"
k_B = 1.3806503e-23
h = 6.62607004e-34
T_d = 18.2
beta = 1.57
...
def dust4(nu, frac_flat):
    dust = (np.exp(c*nu_ref)-1.) / (np.exp(c*nu)-1.) * (nu/nu_ref)**(3.0)* (np.tanh(c*frac_flat*nu)/np.tanh(c*frac_flat*nu_ref))
    return dust
...
A = 1.1
plt.loglog(nu/1e9,A*dust(nu), label = r'MBB $\beta=1.6$')

for i in [6]:
    plt.loglog(nu/1e9,A*dust2(nu,i),label=r'M-MBB $\beta=2.0$, $\alpha =$ '+str(i))


plt.loglog(nu/1e9,A*dust3(nu,6),label=r'M-MBB $\beta=2.6$, $\alpha =$ '+str(6))
"""
...
#plt.loglog(f, d,"-o", label="Template")
#plt.loglog(f_corr, d2,":2",color="C3", label="Fitted amplitudes")
plt.loglog(meanf, meand,":2",color="C6", label="Fitted amplitudes")
...
plt.text(30.,ymin-0.02, "30", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
plt.text(44.,ymin-0.02, "44", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
plt.text(70.,ymin-0.02, "70", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
plt.text(100.,ymin-0.02, "100", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
plt.text(143.,ymin-0.02, "143", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
#plt.text(217.,ymin, "217", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
#plt.text(353.,ymin, "353", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
#plt.text(545.,ymin, "545", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
#plt.text(857.,ymin, "857", color=color, alpha=0.5, ha='center', va='top', fontsize=12)
...
#plt.xlim([0.4,900])
plt.xlim([28,175])
plt.ylim([1e-2, ymin])#100000])
plt.savefig("dustspectra_uten.pdf")
plt.show()
